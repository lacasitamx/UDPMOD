...
BINARY_URL="https://github.com/lacasitamx/UDPMOD/raw/main"
...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - lacasitamx-UDP server install script"
	echo
	echo -e " COMANDO DE EJEMPLO DE USO:"
	echo
	echo -e "$(tbold)Install Lacasitamx-UDP$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified Lacasitamx-UDP binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove Lacasitamx-UDP$(treset)"
	echo -e "\t$0 --remove"
	echo
...
[Unit]
Description=UDPMOD Service BY @lacasitamx
...
tpl_etc_udpmod_config_json() {
  cat << EOF >/etc/udpmod/config.json
{
  "listen": "$UDP_PORT",
  "protocol": "$PROTOCOL",
